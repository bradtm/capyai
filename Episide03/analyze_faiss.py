...
def analyze_faiss_index(faiss_path, num_entries=3, truncate_content=True):
...
Examples:
  ...
  %(prog)s ./custom_index --entries 10 --search "AI"
    ...
    # Main analysis
    analyze_faiss_index(args.faiss_path, args.entries, not args.no_truncate)
    ...
    if args.search:
        ...
        print(f"\n" + "=" * 80)
        print("TIP: Use --search 'query' to test search functionality")
        print(f"Example: python {sys.argv[0]} --search 'your search query'")
