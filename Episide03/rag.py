...
# Validate arguments
if len(sys.argv) != 2:
    print(f"Syntax: {sys.argv[0]} <MediaDirectory|URL>")
    print("Examples:")
    print(f"  {sys.argv[0]} ./media_files")
    print(f"  {sys.argv[0]} https://example.com/article")
    sys.exit(1)

INPUT = sys.argv[1]

# Determine if input is URL or directory
def is_url(string):
    try:
...
if IS_URL:
    ...
    if not os.path.isdir(MEDIA_DIR):
        ...
        sys.exit(1)
    print(f"*** Processing directory: {MEDIA_DIR} ***")
...
# Process input (either URL or media files)
if IS_URL:
    # Process single URL
    url = INPUT
...
if new_files_processed > 0 and len(all_documents) > 0:
    ...
    if IS_URL:
        print(f"\n*** URL already processed or failed to extract content ***")
    else:
        if use_pinecone:
